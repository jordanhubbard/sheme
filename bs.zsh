...
bs() {
    ...
    local -A __bs_closure_params=() __bs_closure_body=() __bs_closure_env=()
    local -A __bs_env=() __bs_env_parent=()
    local __bs_heap_next=0
    local __bs_env_next=1
    local __bs_last=""
    local __bs_ret=""
        ...
        case "$head" in
            ...
            'y:set!')
                ...
                if [[ "$env" == '0' ]] && __bs_valid_bash_id "${sv:2}"; then
                    ...
                    local _cur="$env"
                    while [[ -n "$_cur" ]]; do
                        if (( ${+__bs_env[${_cur}:${sv:2}]} )) && [[ "$_cur" == '0' ]]; then
    ...
    {
        printf '__bs_heap_next=%q\n' "$__bs_heap_next"
        printf '__bs_env_next=%q\n'  "$__bs_env_next"
        for _sk in "${(k)__bs_car[@]}"; do
            printf '__bs_car[%s]=%q\n' "$_sk" "${__bs_car[$_sk]}"
        done
        for _sk in "${(k)__bs_cdr[@]}"; do
            printf '__bs_cdr[%s]=%q\n' "$_sk" "${__bs_cdr[$_sk]}"
        done
        for _sk in "${(k)__bs_closure_params[@]}"; do
            printf '__bs_closure_params[%s]=%q\n' "$_sk" "${__bs_closure_params[$_sk]}"
        done
        for _sk in "${(k)__bs_closure_body[@]}"; do
            printf '__bs_closure_body[%s]=%q\n' "$_sk" "${__bs_closure_body[$_sk]}"
        done
        for _sk in "${(k)__bs_closure_env[@]}"; do
            printf '__bs_closure_env[%s]=%q\n' "$_sk" "${__bs_closure_env[$_sk]}"
        done
        for _sk in "${(k)__bs_env[@]}"; do
            printf '__bs_env[%s]=%q\n' "$_sk" "${__bs_env[$_sk]}"
        done
    ...
    # Compute display form of last value (while heap is still available)
    __bs_display "$__bs_last"
    local _last_display="$__bs_ret"
